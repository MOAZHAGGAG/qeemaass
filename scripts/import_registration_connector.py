...
def wait_for_debezium(max_retries=30, delay=5):
    ...
    for attempt in range(max_retries):
        try:
            response = requests.get("http://localhost:8090/connectors", timeout=10)
...
def create_registration_connector():
    ...
    try:
        ...
        response = requests.get("http://localhost:8090/connectors/registration-connector")
        if response.status_code == 200:
            print("🔄 Registration connector already exists, deleting...")
            delete_response = requests.delete("http://localhost:8090/connectors/registration-connector")
        ...
        response = requests.post(
            "http://localhost:8090/connectors",
        ...
        if response.status_code == 201:
            ...
            time.sleep(3)
            status_response = requests.get("http://localhost:8090/connectors/registration-connector/status")
